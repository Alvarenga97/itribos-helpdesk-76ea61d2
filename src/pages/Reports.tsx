...
const insights = [
  ...
  { icon: AlertTriangle, title: 'Categoria crítica: Rede', desc: 'Chamados de rede aumentaram 45% esta semana.', type: 'warning' as const },
  { icon: Users, title: 'Performance destaque', desc: 'Carlos Mendes resolveu 12 chamados com CSAT médio de 4.8.', type: 'info' as const },
  { icon: Clock, title: 'Primeiro contato', desc: 'Tempo médio de primeira resposta: 22 min. Dentro da meta.', type: 'success' as const },
];
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="hidden sm:table-cell px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground">SLA Violado</th>
              <th className="hidden sm:table-cell px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground">CSAT</th>
              <th className="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {reports.map((r, i) => (
              <motion.tr key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="hidden sm:table-cell px-5 py-3.5 font-mono text-sm">
                  <span className={r.slaBreached > 0 ? 'text-destructive' : 'text-muted-foreground'}>{r.slaBreached}</span>
                </td>
                <td className="hidden sm:table-cell px-5 py-3.5 font-mono text-sm text-warning">{r.csat}</td>
                <td className="px-5 py-3.5">
                  <span className={`text-xs font-medium ${r.status === 'Gerado' ? 'text-primary' : 'text-success'}`}>{r.status}</span>
                </td>
              </motion.tr>
            ))}
          </tbody>
