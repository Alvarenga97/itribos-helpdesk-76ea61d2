      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-xl font-bold font-display text-foreground">{ticket.title}</h1>
          </div>
          {ticket.status !== 'CLOSED' && ticket.status !== 'RESOLVED' && (
            <button className="shrink-0 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
              Resolver Chamado
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <div className="rounded-lg border border-warning/30 bg-warning/5 p-5">
                ...
                <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {[
                    ...
                    <div key={block.letter} className="rounded-md border border-border bg-card p-4">
                      <div className="flex items-center gap-2">
              ...
              {comments.length > 0 ? (
                <div className="mt-4 space-y-4">
                  {comments.map(comment => (
                    <div key={comment.id} className={cn('rounded-md border p-4', comment.isInternal ? 'border-warning/20 bg-warning/5' : 'border-border bg-secondary/50')}>
                      <div className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-border card-gradient p-5 space-y-4">
